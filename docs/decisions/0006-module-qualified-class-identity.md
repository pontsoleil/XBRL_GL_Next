...
7. FSM and BSM add `associated_module` and retain `associated_class`.
   ...
   and hierarchy.
8. Both reference fields are required for Association and Specialisation rows,
...
13. A Specialisation super Class is identified by
    `(associated_module, associated_class)`.
...
19. If a Specialisation super Class cannot be resolved uniquely, the complete
...
22. When an Aligned Class specializes a Shared Class, the selected concrete
    ...
    used during Specialisation but is not emitted as a second same-named Class
    in that HMD.
...
- `specialisation.py`, `graphwalk.py`, their fixtures and the taxonomy
