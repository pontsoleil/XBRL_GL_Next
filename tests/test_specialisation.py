...
SPEC = importlib.util.spec_from_file_location("specialisation_revised", SCRIPT)
...
class SpecialisationTests(unittest.TestCase):
    ...
    def run_model(self, rows: list[dict[str, str]], **kwargs):
        ...
        bsm = root / "bsm.csv"
        self.write(fsm, rows)
        processor = MODULE.Specialisation(
            [fsm],
            bsm,
            module_abbreviations={"tst": "TS", "alt": "AL"},
            **kwargs,
        )
        output = processor.specialisation()
        ...
        for value in ("specialisation", "SPECIALISATION", "Specializtion"):
            with self.subTest(value=value):
                ...
                with self.assertRaisesRegex(
                    MODULE.SpecialisationError, "unsupported property_type"
                ):
                    self.run_model(rows)
    ...
    def test_multiplicity_contract_accepts_valid_and_rejects_invalid(self):
        ...
        for value in ("", "0.*", "1.*", "0...*", "..", "*", "0..2"):
            with self.subTest(invalid=value):
                ...
                rows[7]["multiplicity"] = value
                with self.assertRaisesRegex(
                    MODULE.SpecialisationError,
                    "required field|invalid multiplicity",
                ):
                    self.run_model(rows)

    def test_property_module_may_differ_from_owning_class_and_is_preserved(self):
        rows = self.basic_rows()
        rows[7]["module"] = "tst"
        _, output, _ = self.run_model(rows)
        customer = [item for item in output if item["class_term"] == "Customer"]
        address = [
            item for item in customer
            if item["property_type"] == "Composition"
            and item["associated_class"] == "Address"
        ]
        self.assertEqual(len(address), 1)
        self.assertEqual(address[0]["module"], "tst")
        ...
        for header in variants:
            with self.subTest(header=header):
                with tempfile.TemporaryDirectory() as directory:
                    ...
                    with self.assertRaisesRegex(MODULE.SpecialisationError, "header mismatch"):
    ...
    def test_undefined_associated_class_is_rejected_without_output(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(MODULE.SpecialisationError, "undefined associated"):
                processor.specialisation()
    ...
    def test_missing_associated_module_is_rejected(self):
        ...
        with self.assertRaisesRegex(MODULE.SpecialisationError, "requires associated_module"):
            self.run_model(rows)
    ...
    def test_qname_is_rejected(self):
        ...
        with self.assertRaisesRegex(MODULE.SpecialisationError, "not QName"):
            self.run_model(rows)
    ...
    def test_duplicate_class_and_property_are_rejected(self):
        ...
        with self.assertRaisesRegex(MODULE.SpecialisationError, "duplicate Class"):
        ...
        with self.assertRaisesRegex(MODULE.SpecialisationError, "duplicate property identity"):
    ...
    def test_association_identity_ignores_property_term(self):
        ...
        with self.assertRaisesRegex(
            MODULE.SpecialisationError, "duplicate property identity"
        ):
            self.run_model(rows)
    ...
    def test_attribute_rejects_association_role(self):
        ...
        with self.assertRaisesRegex(
            MODULE.SpecialisationError, "must not define association_role"
        ):
            self.run_model(rows)

    def test_specialisation_cycle_is_rejected(self):
        rows = [
            row(sequence="1", level="1", property_type="Class", module="tst",
                class_term="A", multiplicity="1"),
        ...
        with self.assertRaisesRegex(MODULE.SpecialisationError, "cycle detected"):
            self.run_model(rows)
