...
> **重要：プロジェクトの位置付け**
>
> 本プロジェクトは、国際的な共同検討を支援するための独立したProof of Concept
> （PoC）です。本リポジトリの文書、モデル、タクソノミ、プログラム及び検証結果は、
> XBRL Japan、XBRL Europe、XBRL International又はその他の組織によって承認された
> 仕様、標準、実装勧告若しくは公式見解ではありません。組織名及び外部標準への言及は、
> 承認、後援、所属又は公式な共同事業を意味しません。すべての成果物は、明示的に
> release済みと表示されない限り、検討中のWorking Draft又はprototypeです。
...
`semantic_path`は論理階層を識別します。`element`はprefixなしのlowerCamelCase
NCNameとし、module内で一意にします。C、A及びREF行はelement必須です。R行は
multiplicity上限が1ならelementを空欄とし、上限が1を超える又は無制限の場合は
dimension用elementを必須とします。
...
| OIM/Palette基準taxonomy | prototype完了 | `2026-12-31`版46ファイルを`taxonomy/oim/prototype/`へ反映 |
| OIM/Palette構文検証 | 選択entry pointの検証完了 | `plt-oim-2026-12-31.xsd`と`plt-all-2026-12-31.xsd`をArelleで検証し、OIM rootをXMLSpyでも検証。ただし完全な仕様適合を意味しない |
...
直近では、syntax binding、profile、release manifest及びtaxonomy generatorへ
進む前に、FSM→BSM→LHM/HMD展開処理の再現可能な基準環境を確立することを
最優先とします。program、fixture、期待結果、試験、実行環境、command、
終了code及びSHA-256を対応する一式として管理します。
...
| --- | --- | --- | --- |
| Arelle CLI | 2.37.77、64-bit | `$env:ARELLE_CMD` | DTS、XBRL instance、OIM／xBRL-CSVの自動検証 |
| Arelle GUI | 2.37.77、64-bit AMD64 | `$env:ARELLE_GUI` | DTS及び検証結果の対話的確認 |
| Arelle組込みPython | 3.14.1 | Arelle同梱 | Arelle内部実行環境 |
| Altova XMLSpy | 2026、64-bit | `$env:XMLSPY_EXE` | taxonomy編集、構造確認及び相互検証 |
...
prefixなしlowerCamelCase NCNameとして生成し、同じmodule内で末端名が重複する場合は
親から先祖の語を重複なく前置します。全先祖を使っても一意にならなければ連番、
入力順又はhashを使わずエラーとします。C、A及びREFはelement必須であり、Rは
multiplicity上限1なら空欄、上限1超又は無制限ならdimension用elementを必須とします。
...
最初に、現在の登録準備計画に従ってFSM→BSM→LHM/HMDの再現可能な基準環境を
確立します。この基準環境を登録し再現確認するまでは、syntax binding、HMD/profile
manifest、release manifest及びtaxonomy generatorの実装を再開しません。
