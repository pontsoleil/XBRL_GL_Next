...
- `C:\Users\nobuy\GitHub\UADA\XBRL-GL-Next`は読み取り専用の調査元とする。
...
- GitHub remote追加、repository作成、公開設定変更及びpushは別途利用者の承認を要する。
...
実施済みcopyのfile-level evidenceは
[`inventory/source-comparison.csv`](inventory/source-comparison.csv)を参照します。
...
| 相対path | 分類 | 57件からの除外理由 | 検証 |
|---|---|---|---|
| `PROVENANCE.md` | WORK生成のprovenance文書 | sourceからコピーしたpayloadではなく、import後にWORKで作成 | source側不存在、WORK SHA-256及び記載source pathを確認 |
...
python .\tools\inventory\generate_phase0_manifests.py `
  --repo . `
  --source C:\Users\nobuy\GitHub\UADA\XBRL-GL-Next `
  --official-dir <公式ZIPを一時取得したdirectory> `
  --work-root C:\Users\nobuy\GitHub\WORK `
...
`OPEN_ISSUES.md`のpublication blockerを解消し、利用者がremote、visibility及び
登録対象を承認した後に行います。
...
`TaxonomyFramework/PRIVATE_GITHUB_BASELINE_REGISTRATION_PLAN.md`を正本とする。
