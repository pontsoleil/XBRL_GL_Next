...
| P0-15 | In progress | LHM and HMD terminology | Graph Walk produces 17-column LHM. HMD is the message-level subset selected from LHM during binding. A single-root LHM is content-identical to that root's HMD | Update remaining documents and validate HMD/binding artifacts when available | Phase 1 |
...
| P0-18 | High | Semantic pipeline integration | FSM 14, BSM 15 and LHM 17 columns are implemented and tested as a candidate set; HMD/binding remains outside Graph Walk | Register the matched Semantic program/test set after approval; continue HMD/binding review separately | Phase 1 |
