...
The formal generated taxonomy package contains **76 files**. Repository documentation such as `README.md` and `README_ja.md` is not included in that count.

## Module-level palette components

Each current module directory (`btx`, `bus`, `cor`, `ehm`, `lnk`, `muc`, `taf`) contains reusable module-level palette components shared by the Tuple and OIM bindings.
